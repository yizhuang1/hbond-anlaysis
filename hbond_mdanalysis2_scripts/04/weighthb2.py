...
from remove_duplicate_pw import *
...
def plot_hb(avgB,st,color,lw):
    ...
    if (st=='01'):
        ...
        np.savetxt('%s_w.dat' % st,hbdata,fmt=['%3.4f','%3.11f'],delimiter=' ')
    elif st !='01':
...
def pack(stage):
    ...
    for path in glob(os.path.join(my_dir,'%s/*/*-hb_pr*resname*.pkl.*' % stage)):
        seed = path.split('.')[-1]
        num_traj=path.split('/')[-2]
        sample_i = pickle.load(open(path,'rb'))
        #name_pp=path.split('/')[-1].replace('resnameTIP3','protein')
        #sample_i_pp = pickle.load(open(os.path.join(os.getcwd(), stage, num_traj, name_pp), 'rb'))
        bond_clist=[]
        #repli_h(sample_i_pp, sample_i, bond_clist)
        #bond_clist=[]
...
#leg=ax.legend()
#plt.yticks(np.arange(0,40,2),fontsize=14)
#plt.gca().set_ylim(ymin=-0.5)
#plt.gca().set_ylim(ymax=40.5)
